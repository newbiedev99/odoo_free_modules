...
window.addEventListener('load', (e) => {
    ...
    onchangePadding(paddingDefault)
})
...
const paperSettingsDialog =  {
    ...
    onSubmit: (api) => {
        ...
        paperSettingsDialog.initialData.paddingTop = paddingTop.toString()
        paperSettingsDialog.initialData.paddingBottom = paddingBottom.toString()
        paperSettingsDialog.initialData.paddingRight = paddingRight.toString()
        paperSettingsDialog.initialData.paddingLeft = paddingLeft.toString()
...
buttonSave.addEventListener('click', (e) => {
    ...
    elementEditor.forEach(editor => {
        ...
        const elemWithStyle = editor.querySelectorAll('[data-mce-style]')
        elemWithStyle.forEach(elem => {
            var attr = elem.getAttribute('data-mce-style')
            elem.setAttribute('style', attr)
            elem.removeAttribute('data-mce-style')
        })
        var html = editor.innerHTML
        html = html.replace(/<br( data-mce-bogus="1")?>/gm, '').replace(/<p><\/p>/gm, '')
        console.log(html)
    })
})
